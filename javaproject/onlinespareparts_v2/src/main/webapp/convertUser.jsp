...
<head>
    ...
    <title>Convert User - Online Spare Parts Store</title>
...
<body>
    ...
    <h1 class="page-title">Convert User Role</h1>
    
    <div class="container">
        <div class="tabs">
            <button class="tab active" onclick="openTab(event, 'toSeller')">Customer to Seller</button>
            <button class="tab" onclick="openTab(event, 'toCustomer')">Seller to Customer</button>
        </div>
        
        <div id="toSeller" class="tab-content active">
            <div class="card">
                <h2 class="card-title">Convert Customer to Seller</h2>
                        ...
                        <tbody id="customerTableBody">
                            ...
                            %>
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-warning" onclick="showConvertModal('customer', '<%= customer.getUsername() %>', '<%= customer.getName() %>')">Convert to Seller</button>
                                    </td>
                                </tr>
                            <% 
                                }
        ...
        <div id="toCustomer" class="tab-content">
            <div class="card">
                <h2 class="card-title">Convert Seller to Customer</h2>
                        ...
                        <tbody id="sellerTableBody">
                            ...
                            %>
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-warning" onclick="showConvertModal('seller', '<%= seller.getUsername() %>', '<%= seller.getName() %>')">Convert to Customer</button>
                                    </td>
                                </tr>
                            <% 
                                }
    ...
    <div class="modal-overlay" id="convertModal">
        <div class="modal">
            <div class="modal-header">
                <h3 class="modal-title" id="convertModalTitle">Convert User</h3>
            </div>
            <div class="modal-body">
                <p id="convert-modal-message">
                    Are you sure you want to convert this user's role?
                </p>
                
                <form id="convertForm" action="AdminServlet" method="get">
                    <input type="hidden" name="action" value="convertUser">
                    <input type="hidden" id="userType" name="userType" value="">
                    <input type="hidden" id="username" name="username" value="">
                    
                    <div class="form-group">
                        <label class="form-label">Confirm user conversion by typing their username below:</label>
                    ...
                    <div id="sellerFields" style="display: none;">
                        ...
                        <div class="form-group">
                            <label class="form-label" for="shopDescription">Shop Description</label>
                            <textarea class="form-control" id="shopDescription" name="shopDescription" rows="3" placeholder="Enter shop description"></textarea>
                        </div>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                ...
                <button class="btn btn-primary" id="confirmConvertBtn" disabled>Convert</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Show convert modal
        function showConvertModal(userType, username, name) {
            // Set modal title and content based on the conversion direction
            let title, message;
            
            if (userType === 'customer') {
                title = "Convert Customer to Seller";
                message = `Are you sure you want to convert <strong>${name}</strong> (${username}) from a customer to a seller?`;
                document.getElementById('sellerFields').style.display = 'block';
            } else {
                title = "Convert Seller to Customer";
                message = `Are you sure you want to convert <strong>${name}</strong> (${username}) from a seller to a customer?`;
                document.getElementById('sellerFields').style.display = 'none';
            }
            
            document.getElementById('convertModalTitle').innerText = title;
            document.getElementById('convert-modal-message').innerHTML = message;
            ...
            // Add event listener for confirmation field
            document.getElementById('confirmUsername').addEventListener('input', function() {
                document.getElementById('confirmConvertBtn').disabled = (this.value !== username);
            });
            
            // Set convert button action
            document.getElementById('confirmConvertBtn').onclick = function() {
                if (document.getElementById('confirmUsername').value === username) {
                    if (userType === 'customer' && document.getElementById('shopName').value.trim() === '') {
                        showToast("Error", "Shop name is required for converting to seller", "error");
                        return;
                    }
                    
                    document.getElementById('convertForm').submit();
                    showToast("Success", "Converting user... Please wait.", "success");
